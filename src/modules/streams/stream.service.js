...
export async function requestStream({ user, deviceId, streamType, viewerSocketId = null, io = null }) {
  ...
  if (existing) {
    return { conflict: true, message: 'An active stream session already exists for this device and type' };
  }
  ...
  if (io) {
    io.to(`device:${deviceId}`).emit('start-stream', {
      sessionId: session.id,
      streamType,
      deviceId,
...
export async function applyStreamAnswer({ sessionId, answer, io = null }) {
  ...
  await session.update({ answer });
  touchSessionActivity(sessionId);

  if (io) {
    io.to(`device:${session.device_id}`).emit('stream-answer', {
      sessionId,
      answer,
      timestamp: now().toISOString(),
    });
...
export async function appendViewerIceCandidate({ sessionId, candidate, io = null }) {
  ...
  if (io) {
    io.to(`device:${session.device_id}`).emit('viewer-ice-candidate', {
      sessionId,
      candidate,
      timestamp: now().toISOString(),
    });
...
export async function appendAgentIceCandidate({ sessionId, candidate, io = null }) {
  ...
  if (io && viewerSocketId) {
    const viewerSocket = io.sockets.sockets.get(viewerSocketId);
    viewerSocket?.emit('agent-ice-candidate', {
      sessionId,
      candidate,
      timestamp: now().toISOString(),
    });
...
export async function getStreamSessionForUser(sessionId, user) {
  ...
  if (role === 'MONITOR' && session.viewer_user_id !== user.id) {
    // Monitors can view their own sessions; admins can view division sessions
    return { forbidden: true };
  }
